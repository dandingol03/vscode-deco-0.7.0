...
class ModuleHandler {
  ...
  readAllComponentLists(dirname) {
    ...
    Logger.info('callback=='+'begin readAllPackageJSON');
    return this.readAllPackageJSON(dirname).then((packageMap) => {
      
      const registries = {}

      // var str='';
      // for(var path in packageMap)
      // str+=path+'\r\n';
      // Logger.info('callback='+'packagemap='+str);
      ...
      _.each(packageMap, (json, filepath) => {
        ...
        if (registry) {
          Logger.info('get deco component================'+filepath);
  ...
  //读取所有组件,path为所打开工程路径,such as /Users/danding/Documents/WebstormProj/ionic-serve-person
  scanPathForRegistries({path}, respond) {
    Logger.info('callback=====scanPathForRegistries');
    this.readAllComponentLists(path).then((registryMap) => {
      var str='';
      for(var field in registryMap)
        str+=field;

      Logger.info('registryMap====='+str);
