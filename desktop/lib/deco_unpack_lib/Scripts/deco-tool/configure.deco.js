...
var adb = require('./util/adb.js');

var DECO = require('deco-tool');
var PROJECT_SETTING = DECO.setting;

var checkEnvironmentOK = function checkEnvironmentOK() {
  if (!process.env.ANDROID_HOME) {
    var defaultSDKPath = '/Users/' + process.env['USER'] + '/Library/Android/sdk';
    try {
    ...
    var tools = path.join(process.env.ANDROID_HOME, 'tools');
    var platformTools = path.join(process.env.ANDROID_HOME, 'platform-tools');
    process.env.PATH = process.env.PATH + ':' + tools + ':' + platformTools;
  } else {
    try {
      fs.statSync(process.env.ANDROID_HOME);
      var _tools = path.join(process.env.ANDROID_HOME, 'tools');
      var _platformTools = path.join(process.env.ANDROID_HOME, 'platform-tools');
      fs.statSync(_tools);
      fs.statSync(_platformTools);
    } catch (e) {
      return false;
    }
  }
...
var checkGenymotionOK = function checkGenymotionOK() {
  if (!process.env.GENYMOTION_APP) {
    var defaultGenymotionPath = '/Applications/Genymotion.app';
    try {
...
var rnCLI = function rnCLI() {
  return path.resolve(__dirname, 'util/rn-cli.js');
};

var androidIsRunning = function androidIsRunning() {
  var emuIsRunning = false;
  try {
    var adbState = child_process.spawnSync('adb', ['get-state'], { env: process.env });
...
DECO.on('list-ios-sim', function (args) {
  ...
  try {
    ...
    return Promise.reject({
      payload: ['No simulators available.', 'Please install Xcode and an iOS simulator to preview your project.']
    });
  }

  var targetAppPath = path.join(process.cwd(), path.dirname(PROJECT_SETTING.iosTarget));
  try {
    fs.statSync(targetAppPath);
  } catch (e) {
    if (e.code == 'ENOENT') {
      return Promise.reject({
        payload: ['iOS simulator cannot launch without building your project.', 'Please hit cmd + B or Tools > Build Native Modules to build your project.', 'If you have a custom build outside of Deco, go to Deco > Project Settings and change the "iosTarget" to your .app file location']
      });
    }
  }
  ...
  text.split('\n').forEach(function (line) {
    ...
    if (device && currentOS) {
      ...
      devices.push({ deviceId: deviceId, name: name, version: currentOS });
    }
  });
  var uniqueSims = _.chain(devices).orderBy('version', 'desc').unionBy('name').reverse().value();

  if (uniqueSims.length == 0) {
    return Promise.reject({
      payload: ['No simulators available.', 'Please open Xcode and download iOS simulators.']
    });
  }

  var iphones = _.filter(uniqueSims, function (s) {
    return s.name.match(/iPhone/);
  });
  var ipads = _.filter(uniqueSims, function (s) {
    return s.name.match(/iPad/);
  });
...
DECO.on('sim-ios', function (args) {
  var targetPath = path.join(process.cwd(), PROJECT_SETTING.iosTarget);
  try {
  ...
  var bundleID = child_process.execFileSync('/usr/libexec/PlistBuddy', ['-c', 'Print:CFBundleIdentifier', path.join(targetPath, 'Info.plist')], { encoding: 'utf8' }).trim();
...
DECO.on('build-ios', function (args) {
  var ext = path.extname(PROJECT_SETTING.iosProject);
  var isWorkspace = ext === '.xcworkspace' ? true : false;

  var xcodebuildArgs = [isWorkspace ? '-workspace' : '-project', path.basename(PROJECT_SETTING.iosProject), '-scheme', PROJECT_SETTING.iosBuildScheme, '-destination', 'id=' + args.deviceId, '-derivedDataPath', 'build'];
...
DECO.on('list-android-sim', function (args) {
  if (!checkEnvironmentOK()) {
    return Promise.reject({
      payload: ['The Android SDK path is missing the "tools" sub-folder.', 'Please install Android Studio if you haven\'t already. (https://facebook.github.io/react-native/docs/getting-started.html#android-studio)', 'Please check preferences (cmd + ,) to make sure your Android SDK path is set to the right location.']
    });
  }
  ...
  if (process.env.USE_GENYMOTION) {
    if (!checkGenymotionOK()) {
      return Promise.reject({
        payload: ['Cannot find your Genymotion Application', 'Check that it is installed, then set the path to your Genymotion in preferences (cmd + ,)']
      });
    }
  ...
  try {
    ...
    if (text == '') {
      return Promise.reject({
        payload: ['You have not created any devices yet.', 'Please open Android Studio and create a new device, you will see the name listed here.']
      });
    }
  } catch (e) {
    return Promise.reject({
      payload: ['The Android SDK path is missing the "tools" sub-folder.', 'Please check preferences (cmd + ,) to make sure your Android SDK path is set to the right location.']
    });
  }
...
DECO.on('reload-android-app', function (args) {
  ...
  var packageName = fs.readFileSync(PROJECT_SETTING.androidManifest, 'utf8').match(/package="(.+?)"/)[1];
    ...
    if (!process.env.USE_GENYMOTION) {
      ...
      switch (args.identifier) {
        case 'run':
          break;
        case 'launch':
          var geny = child_process.spawn('open', ['/Applications/Genymotion.app']);
...
DECO.on('init-template', function (args) {
  fs.createReadStream(path.join(__dirname, 'template.configure.deco.js')).pipe(fs.createWriteStream(path.join(process.cwd(), 'configure.deco.js')));
  return Promise.resolve();
});